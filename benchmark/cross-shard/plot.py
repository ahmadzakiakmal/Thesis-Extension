...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Compare cross-shard results with same-shard baseline
  python plot.py \\
    --cross-shard ./records/record-group \\
    --same-shard ../latency/records/08-10
  ...
  python plot.py \\
    --cross-shard ./records/record-group \\
    --same-shard ../latency/records/08-10 \\
    --configs 4-2,4-3,4-4,4-5

Note: Both --cross-shard and --same-shard directories are REQUIRED.
      --cross-shard typically points to ./records/record-group
      --same-shard typically points to ../latency/records/<date>
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Directory containing cross-shard benchmark CSV files (typically ./records/record-group)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Directory containing same-shard/direct latency benchmark CSV files (baseline from latency benchmark)'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output directory for plots (default: same as cross-shard directory)'
    )
    ...
    # Set output directory to cross-shard directory if not specified
    if args.output is None:
        args.output = args.cross_shard
    
    # Validate directories exist
    if not os.path.exists(args.cross_shard):
        print(f"❌ ERROR: Cross-shard directory '{args.cross_shard}' does not exist!")
        return 1
    
    if not os.path.exists(args.same_shard):
        print(f"❌ ERROR: Same-shard directory '{args.same_shard}' does not exist!")
        return 1
