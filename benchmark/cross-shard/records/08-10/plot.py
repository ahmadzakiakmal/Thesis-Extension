...
def calculate_average_latency(csv_files, exclude_steps=None):
    """
    Calculate average latency from CSV files.
    ...
    Returns:
        Dict of config -> average latency
    """
...
def plot_comparison(same_shard_data, cross_shard_data, output_dir='.', output_prefix='shard_comparison'):
    """
    Create comparison plot between same-shard and cross-shard operations.
    """
    # Get configs that exist in both datasets
    configs = sorted([c for c in same_shard_data.keys() if c in cross_shard_data.keys()])
...
Examples:
  ...
  python compare_shards.py \\
    ...
    parser.add_argument(
        '--configs',
        type=str,
        default='4-1,4-2,4-3,4-4',
        help='Comma-separated list of configurations to compare (default: 4-1,4-2,4-3,4-4)'
    )
    ...
    cross_shard_data = calculate_average_latency(
        cross_shard_files, 
        exclude_steps=['Commit Session', 'Complete Workflow']
    )
    same_shard_data = calculate_average_latency(
        same_shard_files, 
        exclude_steps=['Commit Session', 'Complete Workflow']
    )
    ...
    plot_comparison(same_shard_data, cross_shard_data, args.output, 'shard_comparison')
